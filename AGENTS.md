...
## Project Overview
JavaFX desktop application (Java 25) with MySQL database backend. Stack: Maven, JUnit 5, JavaFX 21.0.6, MySQL Connector, BCrypt for password hashing.
...
**Error Handling**: Declare `throws SQLException` for database methods. Check null explicitly. Use BCrypt for passwords (never plaintext).

**DTO/Model Separation**: DTOs for data transfer, Models for DB operations. Keep business logic in Models.

**Database**: Use PreparedStatements (prevent SQL injection). Implement Singleton for DB connections: `DBConnection.getInstance()`.

**Security**: Never hardcode DB credentials—store externally. Hash passwords with BCrypt. Validate user input in Controllers.
